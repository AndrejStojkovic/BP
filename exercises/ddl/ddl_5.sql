...
-- Lista_zelbi(naslov*, k_ime*, ime*), PK: naslov, k_ime, ime
-- Preporaka(ID, k_ime_od*, k_ime_na*, naslov*, datum, komentar, ocena), PK: ID

-- Additional requirements:
-- 1. We want to keep track of the Video entries (Video_zapis) and the us ers that are deleted from the database.
-- 2. User (Korisnik) cannot recommend (Preporaka) himself a video.
-- 3. The date of a recommendation (Preporaka) cannot be before the current DATE().
...
CREATE TABLE IF NOT EXISTS Preporaka (
    ...
    FOREIGN KEY (naslov)
        REFERENCES Video_zapis(naslov)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CHECK (k_ime_od != k_ime_na AND datum < DATE())
);
